...
public class Bartok : MonoBehaviour {
	...
	void Start () {
		...
		//The ref keyword passes a reference to deck.cards, which
	...
	void LayoutGame(){
		...
		if (layoutAnchor == null) {
			//Creates a empty GameObject named LayoutAnchor
		...
		for (int i = 0; i <numStartingCards; i++) {
			...
			for(int j = 0; j<4; j++){ //There are always four players
				...
				//By setting the time Start before calling AddCard, we 
				//ovveride the automatic setting of timeStart in CardBartok.MoveTo()

				//Add the card to the player's hand. The modulus (%4)
				//reults in a number from 0 to 3
...
//	}
	...
	public void CardClicked(CardBartok tCB){
		//If its not the human's turn dont respond
		if(CURRENT_PLAYER.type != PlayerType.human){
			return;
		}

		//If the game is waiting on a card to move don't respod
		if (phase == TurnPhase.waiting)
			return;

		//Act differently based on whether it was a card in hand or 
		//on the drawPile that was clicked
		switch (tCB.state) {
