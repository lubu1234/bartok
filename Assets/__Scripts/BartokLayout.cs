...
public class BartokLayout : MonoBehaviour {
	...
	public void ReadLayout(string xmlText){
		...
		for (int i = 0; i<slotsX.Count; i++) {
			...
			//this converts the number of hte layerId into a text layerName
			tSD.layerName = tSD.layerID.ToString();

			//The layers are used to make sure that correct cards are
			//on top of the others. In unity2d all of our asses are effectively at the same
			//z-depth, so sorting layers are used to differentiate between them
			...
			case "drawpile":
				//The drawPile x stagger is read but not actually used in Bartok
