...
public class Player{
	...
	public CardBartok AddCard(CardBartok eCB){
		...
		if (type == PlayerType.human) {
			...
			//It is similar to doing a foreach and sorting them by rank
			...
			//but sine we're only doing it once every turn it isn't a problem
		}
	...
	public void FanHand(){
		...
		for(int i = 0; i<hand.Count;i++){
			//Adds the rotations of the different player's hands
			rot = startRot - Bartok.S.handFanDegrees*i; // Rotate about the z axis

			rotQ = Quaternion.Euler(0,0,rot); //Quaternion representing the same rotation as rot

			//pos is a vector3 half a card height above [0,0,0] (ie [0,1.75,0])
			pos = Vector3.up * CardBartok.CARD_HEIGHT/2f;

			//Multiplying a Quaternion by a Vector3 rotates that Vector3 by the rotation
			// stored in the Quaternion. The result give sus a vector above [0,0,0] that has been
			...
			//This stagggers the cards in the z direction, which isn't visible
			//but which does keep their colliders from overlapping
			pos.z = -0.5f*i;

			//The line below makes sure that the card starts moving immediately
			//if its not the initial deal at the beginning of the game
			if(Bartok.S.phase != TurnPhase.idle){
				hand[i].timeStart = 0;
			}

			//Set the localPosition and rotation of the iterated card in the hand
			hand[i].MoveTo(pos,rotQ); //Tell the CardBartok to interpolate
...
//			hand[i].state = CBState.hand;
			...
			//So if (type == PlayerType.human, hand[i].faceUp is set tp true
