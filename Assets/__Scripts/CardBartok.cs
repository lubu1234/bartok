...
//CBState include both states for the game and to_states for movement
public enum CBState{
	drawpile,
	toHand,
	hand,
...
public class CardBartok : Card {
	...
	public void MoveTo(Vector3 ePos, Quaternion eRot){
		//Make new interpolation for this card
		...
		//otherwie, it starts at timeStart. This way, if timeStart is already set,
	...
	void Update () {
		switch (state) {
		//All the to_states are ones where the card is interpolating
		case CBState.toHand:
		...
		case CBState.to:
			...
			if(u<0){ 
				//If u<0 then we shouldnt move yet
				//stay at the initial position
				transform.localPosition = bezierPts[0];
				transform.rotation = bezierRots[0];
				return;
			}else if(u>=1){
				//if u >=1 were finished moving
				...
				//Reset timeStart to 0 so its gets overwritten next time
				timeStart = 0;

				if(reportFinishTo != null){
					//If theres a callback GameObject then use SendMessage to call the CBCallback method
					//with this as the parameter
					reportFinishTo.SendMessage("CBCallback",this);

					//After callng SendMessage(), reportFinishTo must be set
					//to null so that the card doesnt continue to report to the same GO
					//every subsequent time it moves
					reportFinishTo = null;

				}else if (callbackPlayer != null){
					//If theres a callback Player
