...
public class Deck : MonoBehaviour {
	...
	public void ReadDeck(string deckXMLText){
		...
		//For more info read about XML in the Use Concepts appendix
		...
		for (int i = 0; i<xCardDefs.Count; i++) {
			...
			//Grab a PT_XMLHashList of all the <pip> on this <card>
			...
			//Face cards (J, Q, & K) have a face attribute
			//cDef.face is the base name of the face card sprite
			//e.g FaceCard_11 is the base name of the Jack face Sprites
	...
	public CardDefinition GetCardDefinitionByRank(int rnk){
		...
		foreach (CardDefinition cd in cardDefs) {
			//If the rank is correct, return the definition
			if(cd.rank == rnk){
				return(cd);
			}
		}
	...
	public void MakeCards(){
		...
		//Each suit goes from 1 to 13 (eg. c1 to c13 for Clubs)
		...
		//Make a List to gold all the cards
		cards = new List<Card> ();
		...
		for(int i = 0; i<cardNames.Count; i++){
			...
			foreach(Decorator deco in decorators){
				...
				if(deco.flip){
					//An Euler rotation 180 degrees around the Z-axis will flip it
	...
	// ref means that the variable passed in is getting changed and not just a param variable
	static public void Shuffle(ref List<Card> oCards){
		//Create a temporary List to hold the new shuffle order
		List<Card> tCards = new List<Card> ();

		int ndx; //This will hold the index of the card to be moved
		tCards = new List<Card> (); //Intialize the temporary List
